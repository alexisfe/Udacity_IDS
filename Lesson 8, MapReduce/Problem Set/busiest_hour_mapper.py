...
def mapper():
    ...
    key1_idx = header.index("UNIT")
    key2_idx = header.index("DATEn")
    key3_idx = header.index("TIMEn")
    val_idx = header.index("ENTRIESn_hourly")

    for line in sys.stdin:
        row = line.strip().split(",")
        print '{0}\t{1}\t{2}\t{3}'.format(row[key1_idx], row[val_idx], row[key2_idx], row[key3_idx])
