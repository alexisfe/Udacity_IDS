...
def predictions(weather_turnstile):
    ...
    features = np.array(weather_turnstile[['rain', 'fog', 'Hour', 'precipi', 'thunder', 'meantempi', 'meandewpti', 'meanpressurei', 'meanwindspdi']])
    target = np.array(weather_turnstile['ENTRIESn_hourly'])
    ols_model = sm.OLS(endog=target, exog=sm.add_constant(features), hasconst='raise').fit()

    print ols_model.params
    print ols_model.tvalues
    print ols_model.rsquared

    y_pred = ols_model.predict(sm.add_constant(features))
